...
bool loadFileInInternalStorage(const char* asset_name,AAssetManager* asset_manager,std::string  & filename, std::string apkInternalPath){

  filename = apkInternalPath + "/" + asset_name;
  FILE* file = fopen(filename.c_str(), "rb");
  if (file ) {
    LOGI("Found extracted file in assets: &s",filename.c_str());
      __android_log_print(ANDROID_LOG_VERBOSE, "APPNAME", "Found extracted file in assets: %s", filename.c_str());
    fclose(file);
    return true;
  }

  bool result = false;
  AAsset* asset = AAssetManager_open(asset_manager, asset_name, AASSET_MODE_STREAMING);
  char buf[BUFSIZ];
  int nb_read = 0;
  if (asset != NULL)
  {
  ...
  {
      LOGI("Asset not found:&s",asset_name);
  }
